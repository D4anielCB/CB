...
def FilmesRC(): #221
	...
	for x in m:
		try:
			...
			url2 = m[0]
			AddDir(meta['title'] +" [COLOR yellow]("+str(meta['year'])+")[/COLOR] "+" [COLOR blue]["+str(meta['rating'])+"][/COLOR]" , url2 + file[0] +"?play|Referer=http://redecanais.video/", 229, isFolder=False, IsPlayable=True, metah=meta)
		except:
			pass
	setViewM()
...
def MoviesRCD(): #90 Filme dublado
	...
	try:
		...
		for x in range(0, 5):
			l +=1
			link = common.OpenURL(proxy+"http://www.redecanais.video/browse-filmes-dublado-videos-"+str(l)+"-"+cOrdRCF+".html")
			if Clista2[int(Cat)] != "Sem filtro (Mostrar Todos)":
				link = common.OpenURL(proxy+"http://www.redecanais.video/browse-"+Clista2[int(Cat)]+"-Filmes-videos-"+str(l)+"-"+cOrdRCF+".html")
			match = re.compile('href=\"([^\"]+).{70,90}src=\"([^\"]+)\".alt=\"([^\"]+)').findall(link)
			if match:
				for url2,img2,name2 in match:
					url2 = re.sub('^\.', "http://www.redecanais.video/", url2 )
...
def MoviesRCL(): #91 Filme Legendado
	...
	try:
		...
		for x in range(0, 5):
			l +=1
			link = common.OpenURL(proxy+"http://www.redecanais.video/browse-filmes-legendado-videos-"+str(l)+"-"+cOrdRCF+".html")
			if Clista2[int(Cat)] != "Sem filtro (Mostrar Todos)":
				link = common.OpenURL(proxy+"http://www.redecanais.video/browse-"+Clista2[int(Cat)]+"-Filmes-Legendado-videos-"+str(l)+"-"+cOrdRCF+".html")
			match = re.compile('href=\"([^\"]+).{70,90}src=\"([^\"]+)\".alt=\"([^\"]+)').findall(link)
			if match:
				for url2,img2,name2 in match:
					url2 = re.sub('^\.', "http://www.redecanais.video/", url2 )
...
def MoviesRCN(): #92 Filmes Nacional
	...
	try:
		...
		for x in range(0, 5):
			l +=1
			link = common.OpenURL(proxy+"http://www.redecanais.video/browse-filmes-nacional-videos-"+str(l)+"-"+cOrdRCF+".html")
			match = re.compile('href=\"([^\"]+).{70,90}src=\"([^\"]+)\".alt=\"([^\"]+)').findall(link)
			if match:
				for url2,img2,name2 in match:
					url2 = re.sub('^\.', "http://www.redecanais.video/", url2 )
...
def MoviesRCR(): # Lancamentos
	...
	try:
		...
		for x in range(0, 5):
			l +=1
			link = common.OpenURL(proxy+"https://www.redecanais.video/browse-filmes-lancamentos-videos-"+str(l)+"-date.html")
			match = re.compile('href=\"([^\"]+).{70,90}src=\"([^\"]+)\".alt=\"([^\"]+)').findall(link)
			if match:
				for url2,img2,name2 in match:
					url2 = re.sub('^\.', "http://www.redecanais.video/", url2 )
...
def PlaySRC(): #133 Play series
	try:
		url2 = re.sub('redecanais\.[^\/]+', "redecanais.video", url.replace("https","http") )
	...
	if background=="None":
		...
		for name2,url2,brp in epi:
			...
			if "<" in namem:
				namem = ""
			if urlm:
				urlm[0] = "http://www.redecanais.video/" + urlm[0] if "http" not in urlm[0] else urlm[0]
			if len(urlm) > 1:
				urlm[1] = "http://www.redecanais.video/" + urlm[1] if "http" not in urlm[1] else urlm[1]
...
def SeriesRC(urlrc,pagina2): #130 Lista as Series RC
	try:
		...
		for x in range(0, 5):
			l +=1
			link = common.OpenURL(proxy+"http://www.redecanais.video/browse-"+urlrc+"-videos-"+str(l)+"-"+cOrdRCS+".html")
			match = re.compile('href=\"([^\"]+).{70,90}src=\"([^\"]+)\".alt=\"([^\"]+)').findall(link)
			if match:
				for url2,img2,name2 in match:
					url2 = re.sub('^\.', "http://www.redecanais.video/", url2 )
		...
		for name2,url2,brp in match:
			...
			if urlm:
				if "http" not in urlm[0]:
					urlm[0] = "http://www.redecanais.video/" + urlm[0]
			if len(urlm) > 1:
				if "http" not in urlm[1]:
					urlm[1] = "http://www.redecanais.video/" + urlm[1]
...
def Busca(): # 160
	...
	try:
		...
		for x in range(0, 6):
			l +=1
			link = common.OpenURL(proxy+"http://www.redecanais.video/search.php?keywords="+d+"&page="+str(l))
			match = re.compile('href=\"([^\"]+).{70,90}src=\"([^\"]+)\".alt=\"([^\"]+)').findall(link)
			if match:
				for url2,img2,name2 in match:
					url2 = re.sub('^\.', "http://www.redecanais.video/", url2 )
...
def PlayMFO1(): #172
	if re.compile('\d+').findall(str ( background )) :
		...
		if sel!=-1:
			link = common.OpenURL(proxy+ url+"?q="+s[sel] )
			ST(link)
	...
	try:
		...
		if m:
			...
			if m2:
				...
				info = i[0] if i else ""
				link3 = common.OpenURL(proxy+ "https:"+m2[0] )
				ST(link3 )
